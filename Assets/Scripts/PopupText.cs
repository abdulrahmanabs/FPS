...
public class PopupText : MonoBehaviour
{
    ...
    public void ShowPopup(Vector3 worldPosition, string message, Color textColor,float waitTime,GameObject lookAt)
    {
        TMP_Text popup = GetPooledPopup();
        SetupPopup(popup, worldPosition, message, textColor,lookAt);
        activePopups.Add(popup);
        StartCoroutine(FadeOutAndMoveUp(popup,waitTime));
    }
    ...
    private void SetupPopup(TMP_Text popup, Vector3 worldPosition, string message, Color textColor,GameObject lookAt)
    {
    ...
    private IEnumerator FadeOutAndMoveUp(TMP_Text popup,float waitTime)
    {
